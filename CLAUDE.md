...
- **For any user-facing module, you MUST also seed a menu entry**: add the entry in `backend/pkg/constants/default_data.go` (`DefaultMenus`) — the platform auto-derives permission codes (e.g. `feature:view`) from `menu.path` via `SyncPermissions`, and binds the menu to `sys:platform_admin` / `sys:tenant_manager` roles via the `Authority` field. Without this step the new module has no permission code and is invisible to dynamic menu. **See `backend/CLAUDE.md` "Step 12: 菜单与权限码" for the full checklist.**
- **When storing a protobuf message via `field.JSON`, NEVER pass the raw `*xxxV1.SomeMessage` if it contains `oneof` / well-known types / `map<string,Msg>`**. Go's `encoding/json` cannot round-trip protobuf oneof — writes silently corrupt the JSON, **reads fail with `cannot unmarshal ... into isXxx_Yyy`**, and the only visible symptom is "list is empty / seed didn't run" (it ran; the read crashed). Wrap with a `XxxSpecField` type that delegates to `protojson` and register a `copier.TypeConverter` pair on the repo mapper. **Mandatory checklist in `backend/CLAUDE.md` "Step 13".**
