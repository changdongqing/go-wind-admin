...
import { bindMethods, isBoolean, isFunction, mergeWithArrayOverride, StateHandler } from "@/utils";
...
export class VxeGridApi {
  ...
  setState(stateOrFn: ((prev: VxeGridProps) => Partial<VxeGridProps>) | Partial<VxeGridProps>) {
    if (isFunction(stateOrFn)) {
      this.store.setState((prev: VxeGridProps) => {
        return mergeWithArrayOverride(stateOrFn(prev), prev);
      });
    } else {
      this.store.setState((prev: any) => mergeWithArrayOverride(stateOrFn, prev));
    }
  }
